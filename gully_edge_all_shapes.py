...
#------------------------------------------------------------------------------#
#Count the number of features in the input file and create a numpp array
# with a corresponding number of rows and three columns. Column one will be the
# FID of the target shape, column two will be the optimal expand value and
# column three will be the profile curvature value corresponding to the selected
# optimal expand value.

n_rows = arcpy.GetCount_management(bas)
rows = n_rows.getOutput(0)
print 'Number of candidate shapes == ' + str(rows)
array_input = float(rows)
gullies = np.zeros([array_input, 3], dtype=float)
...
for row in cursor:
    ...
    min_value_in = float(min_value)
    values.append(min_value_in)
    print values
    ...
    for i in range(0, 4):
        ...
        if i == 0: #For the first loop iteration, the file to be expanded will just be the input stream order file (or one stream order from that file).
            ...
            print 'Sleeping for one second...'; time.sleep(1)
#------------------------------------------------------------------------------#
    print 'Final value set' + ' - ' + str(values)
